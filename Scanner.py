...
class MainWindow(QMainWindow):
        ...
        if ai == 'All Hosts':
            if selected == 'all':
                for ip in MainWindow.ips:
                    t = threading.Thread(target=MainWindow.port_scan_all, args=[self, ip])
                    t.start()
            elif selected == 'singal port':
                for ip in MainWindow.ips:
                    p = self.spinBox_3.value()
                    t = threading.Thread(target=MainWindow.port_scan_singal, args=[self, ip, str(p)])
                    t.start()
            elif selected == 'range':
                for ip in MainWindow.ips:
                    ...
                    if fp < lp:
                        ...
                        range = str(lp)
                    t = threading.Thread(target=MainWindow.port_scan_range, args=[self, ip, range])
                    t.start()
            elif selected == 'common':
                for ip in MainWindow.ips:
                    t = threading.Thread(target=MainWindow.port_scan_common, args=[self, ip])
                    t.start()
    ...
    def port_scan_all(self, ip):
        out = False
        nm = nmap.PortScanner()
        nm.scan(hosts=ip, arguments='nmap -p 1-65535 -Pn')
    ...
    def port_scan_range(self, ip, range):
        out = False
        nm = nmap.PortScanner()
        nm.scan(ip, range)
        if len(nm.all_hosts()) == 0:
    ...
    def port_scan_common(self, ip):
        out = False
        nm = nmap.PortScanner()
        nm.scan(ip, MainWindow.tcp)
        if len(nm.all_hosts()) == 0:
            out = True
        #    MainWindow.evt_update(self, 0)
        if out:
    ...
    def port_scan_singal(self, ip, p):
        out = False
        nm = nmap.PortScanner()
        nm.scan(ip, p)
        if len(nm.all_hosts()) == 0:
        ...
        if not out:
            for proto in nm[ip].all_protocols():
                if nm[ip][proto][int(p)]['state'] == 'open' or nm[ip][proto][int(p)]['state'] == 'filtered':
                    pp = Port(p, nm[ip]['tcp'][int(p)]['name'], nm[ip]['tcp'][int(p)]['product'],
                              nm[ip]['tcp'][int(p)]['version'])
                    MainWindow.add_child(self, ip, pp)
                    print('done  ' + ip)
        #           MainWindow.evt_update(self, 0)
        print(ip + '  Scan Complete')
